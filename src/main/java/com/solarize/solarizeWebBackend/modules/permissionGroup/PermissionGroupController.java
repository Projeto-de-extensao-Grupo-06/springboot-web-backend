...
public class PermissionGroupController {
    ...
    @PreAuthorize("hasAuthority('ROLE_ADMIN')")
    @GetMapping
    public ResponseEntity<Page<GetPermissionGroupDto>> getAll(
            @RequestParam(required = false) String search,
            @RequestParam(required = false) String mainModule,
            @PageableDefault(page = 0, size = 30) Pageable pageable
    ) {
        return ResponseEntity.ok(service.findAll(search, mainModule, pageable));
    }
}
